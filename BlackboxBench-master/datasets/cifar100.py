...
class CIFAR100Data(object):
    ...
    def __init__(self, path):
        train_filenames = ['cifar-100-python/train']
        eval_filename = 'cifar-100-python/test'
        metadata_filename = 'cifar-100-python/meta'
        ...
        for ii, fname in enumerate(train_filenames):
            ...
            train_images[ii * 50000: (ii + 1) * 50000, ...] = cur_images
            train_labels[ii * 50000: (ii + 1) * 50000, ...] = cur_labels
        eval_images, eval_labels = self._load_datafile_test(
    ...
    def _load_datafile(filename):
        with open(filename, 'rb') as fo:
            ...
            image_data = image_data.reshape((50000, 3, 32, 32)).transpose(0, 2, 3, 1)
            return image_data, np.array(data_dict[b'fine_labels'])

    @staticmethod
    def _load_datafile_test(filename):
        with open(filename, 'rb') as fo:
            if version.major == 3:
                data_dict = pickle.load(fo, encoding='bytes')
            else:
