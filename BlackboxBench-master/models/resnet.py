...
class ResNet(nn.Module):
    def __init__(self, depth, num_classes=100, leaky_relu=False):
...
class ResNet18(nn.Module):
    def __init__(self, depth, num_classes=100, leaky_relu=False):
        ...
        self.avgpool = nn.AvgPool2d(4)  # original 8
    ...
    def get_features(self, x, layer, before_relu=False):
        ...
        x = self.layer3(x)
        if layer > 1 + 3*layers_per_block and layer <= 1 + 4*layers_per_block:
            relative_layer = layer - (1 + 3*layers_per_block)
            x = self.layer_block_forward(x, self.layer4, relative_layer, before_relu=before_relu)
            return x
    ...
    def layer_block_forward(self, x, layer_block, relative_layer, before_relu=False):
        ...
        if relative_layer == 7:
            return layer_block[3](out, before_relu, intermediate=True)

        if relative_layer == 8:
            return layer_block[3](out, before_relu, intermediate=False)
