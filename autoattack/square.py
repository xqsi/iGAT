...
class SquareAttack():
    ...
    def perturb(self, x, y=None):
        ...
        x_norm = (x - self.mean_norm) / self.std_norm
        if not self.targeted:
            acc = self.predict(x_norm).max(1)[1] == y
        else:
            acc = self.predict(x_norm).max(1)[1] != y
        ...
        for counter in range(self.n_restarts):
            ...
            if ind_to_fool.numel() != 0:
                ...
                adv_curr_norm = (adv_curr - self.mean_norm) / self.std_norm
                output_curr = self.predict(adv_curr_norm)
                if not self.targeted:
