...
class AutoAttack():
    def __init__(self, model, norm='Linf', eps=.3, seed=None, verbose=True,
                 ...
                 device='cuda', log_path=None, mean_norm=None, std_norm=None):
        self.model = model
        self.norm = norm
        ...
        if not self.is_tf_model:
            ...
            self.apgd = APGDAttack(self.model, n_restarts=5, n_iter=100, verbose=False,
                ...
                device=self.device, logger=self.logger, mean_norm=mean_norm, std_norm=std_norm)
            
            from .fab_pt import FABAttack_PT
            self.fab = FABAttack_PT(self.model, n_restarts=5, n_iter=100, eps=self.epsilon, seed=self.seed,
                norm=self.norm, verbose=False, device=self.device, mean_norm=mean_norm, std_norm=std_norm)
        
            from .square import SquareAttack
            self.square = SquareAttack(self.model, p_init=.8, n_queries=5000, eps=self.epsilon, norm=self.norm,
                n_restarts=1, seed=self.seed, verbose=False, device=self.device, resc_schedule=False, mean_norm=mean_norm, std_norm=std_norm)
            ...
            self.apgd_targeted = APGDAttack_targeted(self.model, n_restarts=1, n_iter=100, verbose=False,
                ...
                logger=self.logger, mean_norm=mean_norm, std_norm=std_norm)
        ...
        if version in ['standard', 'standard_4', 'plus', 'rand']:
            self.set_version(version)
        
    def get_logits(self, x):
        if not self.is_tf_model:
            x_norm = (x - self.mean_norm) / self.std_norm
