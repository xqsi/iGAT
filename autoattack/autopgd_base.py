...
class APGDAttack():
        ...
        for _ in range(self.eot_iter):
            if not self.is_tf_model:
                with torch.enable_grad():
                    x_adv_norm = (x_adv - self.mean_norm) / self.std_norm
    ...
    def perturb(self, x, y=None, best_loss=False, x_init=None):
        ...
        if not self.is_tf_model:
            ...
            y_pred = self.model(x_norm).max(1)[1]
        else:
        ...
        for target_class in range(2, self.n_target_classes + 2):
            for counter in range(self.n_restarts):
                ...
                if ind_to_fool.numel() != 0:
                    ...
                    if not self.is_tf_model:
                        x_to_fool_norm = (x_to_fool - self.mean_norm) / self.std_norm
                        output = self.model(x_to_fool_norm)
                    else:
